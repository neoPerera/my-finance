  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      const response = await Axios.post(
        `${window.env?.REACT_APP_API_URL}myfinance/reference/ref-accounts/addaccount`,
        formData
      );
      
      console.log("Response:", response.data);
      showMessage("success", "Account saved successfully!");
  ...
  const getSequence = async () => {
    ...
    try {
      const response = await Axios.get(
        `${window.env?.REACT_APP_API_URL}myfinance/reference/ref-accounts/getsequence?type=key`
      );
