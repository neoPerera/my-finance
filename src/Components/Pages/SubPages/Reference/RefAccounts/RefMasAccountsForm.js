...
function RefMasAccountsForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await Axios.post(
        `${window.env?.REACT_APP_API_URL}api/myfinance/reference/ref-accounts/add`,
        formData
      );
      // if (swalInstance) {
    ...
    const getSequence = async () => {
      ...
      try {
        const response = await Axios.get(
          `${window.env?.REACT_APP_API_URL}api/myfinance/reference/ref-accounts/getsequence`
        );
  ...
  return (
    <>
      ...
      <Title level={2}>Accounts Master form</Title>
      <Form
        form={form}
        name="validateOnly"
        layout="vertical"
